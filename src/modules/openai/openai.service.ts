...
export class OpenAiService {
  ...
  generateWritingTask(part: number): Observable<{ data: string }> {
    let result = '';
    return new Observable((subscriber) => {
      this.openAiService.chat.completions
        .create({
          ...
          messages: [
            {
              role: 'system',
              content: `You are an IELTS test maker. You create a test for the IELTS Writing - Part ${part} test according to international standards. Only create the test, no create any more`,
            },
            {
              role: 'user',
              content: `Create a test for the IELTS Writing - Part ${part} test according to international standards. The random seed for this task is ${Math.random()} `,
            },
          ],
          stream: true,
        })
