...
export class SubmissionService extends BaseServiceAbstract<SubmissionModel> {
  constructor(private readonly shareService: ShareService, private submissionRepository: SubmissionRepository) {
    super(submissionRepository);
  }

  async createSubmission(assignment_uid: string, name: string, essay: string, result: string) {
    return this.create({
      uid: this.shareService.makeid(),
      slug: this.shareService.slugify(`${name}-${assignment_uid}-${Date.now()}`),
      assignment_uid,
      name,
      essay,
      result,
